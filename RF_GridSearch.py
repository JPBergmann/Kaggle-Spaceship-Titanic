...
rf_params = {}
# Pipe parameters
rf_params["Preprocessing__Binning__ohe__n_bins"] = [5, 10]
rf_params["Preprocessing__Numeric__Impute__strategy"] = [
    "most_frequent",
    "median",
    "mean",
]
rf_params["Feature_Selection__threshold"] = [0, 0.25]
# Model specific parameters
rf_params["Classifier__n_estimators"] = [100, 250, 500]
rf_params["Classifier__criterion"] = ["gini", "entropy"]
rf_params["Classifier__max_depth"] = [None, 10, 20, 30]
rf_params["Classifier__min_samples_split"] = [2, 5, 10]
rf_params["Classifier__min_samples_leaf"] = [1, 2, 4]
rf_params["Classifier__max_features"] = ["sqrt", "log2"]
# rf_params["Classifier__bootstrap"] = [True, False]
# rf_params["Classifier__class_weight"] = [None, "balanced"]
rf_params["Classifier__max_leaf_nodes"] = [None, 5, 10, 20]
rf_params["Classifier__min_impurity_decrease"] = [0.0, 0.1, 0.2]
...
pred = best_model.predict(test.drop(columns=["Name", "PassengerId", "Cabin"])).astype(
    bool
)
pred_results = test[["PassengerId"]]
pred_results["Transported"] = pred
save_df(data=pred_results, location="RF/GridSearch")
